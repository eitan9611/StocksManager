...
namespace Backend.Services
{
    public class TradeService
    {
        ...
        public async Task<bool> BuyStockAsync(string userEmail, string symbol, int quantity)
        {
            ...
            var hey = await _stockService.GetStockPriceAsync(symbol); // Fetch real price
            Console.WriteLine(hey);
            decimal stockPrice = await _stockService.GetStockPriceAsync(symbol); // Fetch real price
            if (stockPrice <= 0) return false;

            var stock = new Stock { Id = 0, Name = "Eitan", Symbol = symbol, Price = stockPrice };
